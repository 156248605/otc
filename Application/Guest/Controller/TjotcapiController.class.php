...
class TjotcapiController extends Controller
{
	...
	private function filecheck_kh($c,$ids,$rids,$my_note){
		if($c['CheckFlag']=='0'){
			...
			if($r>=1){
				...
				if($r>=1){
					D('CfIvsRightLog')->addRightLogsByFilename($c['FileName'],2,3,1,1,0,"开户文件资料审核机构已收到"); 
			...
			if($r>=1){
				...
				if($r==1){
					D('CfIvsRightLog')->addRightLogsByFilename($c['FileName'],2,1,1,1,0,"开户文件资料审核机构未收到"); 
					$my_note.=",cf_ivs_right表修改成功";
				}else{
					$my_note.=",cf_ivs_right表修改失败";
				}
	...
	private function filecheck_rj($c,$ids,$rids,$my_note){
		...
		if($c['CheckFlag']=='0'){
						...
						if($r>=1){
							...
							if($r>=1){
								D('CfIvsRightLog')->addRightLogsByFilename($c['FileName'],12,13,1,1,0,"入金文件资料审核机构已收到"); 
								$my_note.=",cf_ivs_right表修改成功";
							}else{
								$my_note.=",cf_ivs_right表修改失败";
							}
						...
						if($r>=1){
							...
							if($r>=1){
								D('CfIvsRightLog')->addRightLogsByFilename($c['FileName'],12,11,1,1,0,"入金文件资料审核机构未收到"); 
								$my_note.=",cf_ivs_right表修改成功";
							}else{
								$my_note.=",cf_ivs_right表修改失败";
							}
	...
	private function filecheck_gh($c,$ids,$rids,$my_note){
		if($c['CheckFlag']=='0'){
						...
						if($r>=1){
							...
							if($r>=1){
								// print_r($c);
								D('CfIvsRightLog')->addRightLogsByFilename($c['FileName'],6,7,1,1,0,"过户文件资料审核机构已收到"); 
								$my_note.=",cf_ivs_right表修改成功";
							}else{
								$my_note.=",cf_ivs_right表修改失败";
							}
						...
						if($r>=1){
							...
							if($r>=1){
								D('CfIvsRightLog')->addRightLogsByFilename($c['FileName'],6,4,1,1,0,"过户文件资料审核机构未收到"); 
								$my_note.=",cf_ivs_right表修改成功";
							}else{
								$my_note.=",cf_ivs_right表修改失败";
							}
	...
	private function fileready_kb($c,$tjotc,&$my_note){
		...
		foreach($content as $thv){
			...
			if(trim($thv['KBFHDM'])!="0"){//表示确权出错 
				//更新开户确权记录状态为开户失败
				$r=D('cf_ivs_right')->where(array("status"=>['in','1'],"id"=>$ivs_right['id'],"step"=>"3"))->save(array("status"=>'3',"step"=>'5')); //OTC开户失败
	...
	*/
	private function fileready_rjhb($c,$tjotc,&$my_note){
		...
		foreach($content as $thv){//RJHBGHDH  申请编号=交易编号
			...
			//插入otc_gb_dbf
			$rjhb_data=['cf_ivs_id'=>$ivs['id'],'cf_ivs_right_id'=>$ivs_right['id'],
			...
			if(trim($thv['RJHBFHDM'])!="0"){//表示确权出错 
				//更新开户确权记录状态为过失败
				$r=D('cf_ivs_right')->where(array("status"=>['in','1'],"id"=>$ivs_right['id'],"step"=>"13"))->save(array("status"=>'1',"step"=>'14')); //OTC入金失败
		...
		// if($list){
			...
			$my_note.=",共含有可以赎回过户数据".count($list)."条";
		// }
		// $tjotc->file_down_over_toOTC($c);		//确认文件已经下载通知 
        if(isset($new_in_id)&&$isture){
			//确认文件已经下载通知
	...
	*/
	private function fileready_gb($c,$tjotc,&$my_note){
		...
		foreach($content as $thv){//GBSQDH  申请编号=交易编号
			...
			//插入otc_gb_dbf
			$gb_data=['cf_ivs_id'=>$ivs['id'],'cf_ivs_right_id'=>$ivs_right['id'],
			...
			if(trim($thv['GBFHDM'])!="0"){//表示确权出错 
				//更新开户确权记录状态为过失败
				$r=D('cf_ivs_right')->where(array("status"=>['in','1'],"id"=>$ivs_right['id'],"step"=>"7"))->save(array("status"=>'3',"step"=>'9')); //OTC过户失败
			...
			$qqh_url="http://apitest.tjotc-clear.com.cn/ConfirmReport";
	...
	private function fileready_dz($c,$tjotc,&$my_note,$type=""){
		...
		foreach($content as $thv){//KBSQDH  申请编号=交易编号
			...
			if($ivs_right_list&&$type=="BD"){
				...
				$my_note.="||当前对账金额($DZCYFE)";
				$my_note.="||所有确权走到8的确权总额($all_quequanchenggongSum)".($DZCYFE==$all_quequanchenggongSum?"true":"false");
				$my_note.="||所有确权走到8的并且已经结束确权总额($all_quequanjieshuSum)".($DZCYFE!=$all_quequanjieshuSum?"true":"false"); 
			   ...
			   //并且当前对账金额 与 所有确权走到8的确权总额相等（说明可以更新）
				if( $DZCYFE==$all_quequanchenggongSum &&( $DZCYFE!=$all_quequanjieshuSum )){
					foreach($ivs_right_list as $ivs_right){
		...
		/* 	
			...
			//插入otc_dz_dbf
			$dz_data=['cf_mast_id'=>0,'cust_id'=>$DZJYZH,
	...
	*天津otc回调接口
	*/
	public function otcapidemo($type="1",$pid='0'){
		 
		$time1=time();
		$t1 = microtime(true);
		$a=new \Common\Lib\Otcapi();//实例化对象 
		$_GET['ParValue']='<?xml version="1.0" encoding="gbk"?><root><OrgCode>SEZB004</OrgCode><FileID>2815</FileID><FileName>TJC_TJC999_SEZB004_20160219_120335_RJHB.DBF</FileName><FileSize>698</FileSize><MD5Code>1175B032B8166C07872F37D6B25D39AC</MD5Code></root>';
		$_GET['ParValue']='<?xml version="1.0" encoding="gbk"?><root><OrgCode>SEZB004</OrgCode><FileID>2817</FileID><FileName>TJC_TJC999_SEZB004_20160219_120326_GB.DBF</FileName><FileSize>1159</FileSize><MD5Code>E0C93AE1BAB4109CDE81C19DB6CE8772</MD5Code></root>';
		$_GET['ParValue']='<?xml version="1.0" encoding="gbk"?><root><OrgCode>SEZB004</OrgCode><FileID>2834</FileID><FileName>TJC_TJC999_SEZB004_20160219_110302_RJHB.DBF</FileName><FileSize>6425</FileSize><MD5Code>717864BB6A119C8BF3037A27306270DB</MD5Code></root>';
		$_GET['ParValue']='<?xml version="1.0" encoding="gbk"?><root>	<OrgCode>SEZB004</OrgCode>	<FileID>2838</FileID>	<FileName>TJC_TJC999_SEZB004_20160219_110313_GB.DBF</FileName>	<FileSize>13809</FileSize>	<MD5Code>62E0AB81D2F900BC05A3F3EEDA1E9EC1</MD5Code></root>';
	...
	*/
	public function otcapi($type="1",$pid='0'){
		...
		$t1 = microtime(true);
		$a=new \Common\Lib\Otcapi();//实例化对象 
		/* $_GET['ParValue']='<?xml version="1.0" encoding="gbk"?><root><OrgCode>SEZB004</OrgCode><FileID>2806</FileID><FileName>TJC_TJC999_SEZB004_20160219_110339_RJHB.DBF</FileName><FileSize>698</FileSize><MD5Code>53451A1C5DFA68620483E9B924443F7F</MD5Code></root>';
		 */
		$c=$a->xmlArrayByString($_GET['ParValue']);
		if($type==1&&$_GET['ParName']=="FileReady"){
			ignore_user_abort(false);

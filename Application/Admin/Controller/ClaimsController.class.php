...
/*		if(I('post.keyword')){
        ...
        //债权名称
        if (I('post.keyword')) {
            $where['d.product_name'] = array('LIKE', "%" . I('post.keyword') . "%");
        }
...
/*		if(I('post.createdate')){
			...
			if(in_array($creatrkey,array(1,2,3))){
				...
				$where ['dat_create']=array('LGT',$createdate);
			}
            $where ['dat_create']=array('EGT',I('post.createdate'));
        }*/
    ...
    public function  claimsManager()
    {
        ...
        if ($_SERVER ['REQUEST_METHOD'] == 'POST' || $_SERVER ['REQUEST_METHOD'] == 'GET') {
           ...
           if (isset($_POST['status'])&&$_POST['status']!='') {
            ...
            //债权名称
            if (I('post.keyword')) {
                $where['a.product_name'] = array('LIKE', "%" . I('post.keyword') . "%");
            }
            //有分页时要返回总条数
           ...
           // $current_page = I('post.current_page') ? I('post.current_page') : 1;
            ...
            $data = $Model->alias('a')->field("b.status as zcb_status_code,a.type,a.id,a.product_name,a.status,b.investment_type,ifnull(c.amt_ct_last,a.amt_cf_inv_price) as amt_ct_last,ifnull((c.dat_modify),'--') as dat_create")->join('LEFT JOIN __CAPITALPOOL__ b ON a.capitalid=b.id')->join('LEFT JOIN __CL_CTL__ c ON a.id=c.cod_cl_id')->where($where)->limit("$limit_1,$page_size")->order("c.dat_modify desc")->select();

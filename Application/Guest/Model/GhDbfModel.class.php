...
class GhDbfModel extends Model{
   ...
   //  @param  info  用户额外信息数据	
   //  @param  file_no   文件序号   
	public function  createDbf($cf_ivs_list,$cf_ivs_right_id,$user,$info = array(),$file_no,$type=1){
		// print_r($user);
		$now = time();
		...
		if($flag&&$mast_list){
			$dbf_data = array(
				...
				substr($cf_ivs_list['product_code'],0,10),                  //备用标志 	 
			);
